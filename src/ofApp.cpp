...
// i got this up to 240 frames with steady on my computer, is a bit skewed
// extreme for real time video executions tho
...
// because c++ vectors are like arrays but with some handier ways to access and
// add and delete contents but on the other hand vectors can have dynamic
// lengths and one can really end up in some trouble with dynamically working
// with framebuffers bc its generally best pratice to allocate memory on the gpu
// for all of yr framebuffers before yr code runs, trying to mess with that on
// the fly could have some interesting appeal to a glitch art style but is not
// going to please others
...
void ofApp::setup() {
  ...
  // or if its all going smooth and u want to try higher resolutions here is
  // where to fuck with that

  cam1Manager.setup(720, 480);
  cam2Manager.setup(720, 480);
...
void ofApp::draw() {
  ...
  auto cam1 = cam1Manager.getInput();
  auto cam2 = cam2Manager.getInput();

  if (cam1 && cam1->isInitialized() && cam1->isFrameNew()) {
    shader_mixer.setUniformTexture("cam1", cam1->getTexture(), 2);
  }
  if (cam2 && cam2->isInitialized() && cam2->isFrameNew()) {
    shader_mixer.setUniformTexture("cam2", cam2->getTexture(), 3);
  }

  shader_mixer.setUniformTexture(
      "fb0",
      pastFrames[(abs(framedelayoffset - fbob - gui->fb0_delay_amount -
                      (int(fbob * c8)) -
                      abs(int(fbob * lfo(gui->fb0_delay_lfo_amp,
                                         fb0_theta_delay, 0)))) -
                  1) %
                 fbob]
          .getTexture(),
  ...
  shader_mixer.setUniformTexture(
      "fb1",
      pastFrames[(abs(framedelayoffset - fbob - gui->fb1_delay_amount -
                      (int(fbob * c21)) -
                      abs(int(fbob * lfo(gui->fb1_delay_lfo_amp,
                                         fb1_theta_delay, 0)))) -
                  1) %
                 fbob]
          .getTexture(),
  ...
  shader_mixer.setUniformTexture(
      "fb2",
      pastFrames[(abs(framedelayoffset - fbob - gui->fb2_delay_amount -
                      abs(int(fbob * lfo(gui->fb2_delay_lfo_amp,
                                         fb2_theta_delay, 0)))) -
                  1) %
                 fbob]
          .getTexture(),
  ...
  shader_mixer.setUniformTexture(
      "fb3",
      pastFrames[(abs(framedelayoffset - fbob - gui->fb3_delay_amount -
                      abs(int(fbob * lfo(gui->fb3_delay_lfo_amp,
                                         fb3_theta_delay, 0)))) -
                  1) %
                 fbob]
          .getTexture(),
  ...
  shader_mixer.setUniform1f(
      "fb0lumakeyvalue",
      gui->fb0_key_value + c6 +
          lfo(gui->fb0_lumakeyvalue_lfo_amp, fb0_theta_lumakeyvalue, 0));
  shader_mixer.setUniform1f("fb0lumakeythresh",
                            gui->fb0_key_threshold +
                                lfo(gui->fb0_lumakeythreshold_lfo_amp,
                                    fb0_theta_lumakeythreshold, 0));
  shader_mixer.setUniform1f(
      "fb0blend",
      gui->fb0_mix + lfo(gui->fb0_mix_lfo_amp, fb0_theta_mix, 0) + c7 * 2.0f);

  shader_mixer.setUniform1i("fb0mix", 2);
  shader_mixer.setUniform1i("fb0_toroid_switch", gui->fb0_toroid_switch);

  hsb_x.set(
      (gui->fb0_hue + lfo(gui->fb0_hue_lfo_amp, fb0_theta_hue, 0)) / 10 +
          c13 * .2f,
      (gui->fb0_saturation +
       lfo(gui->fb0_saturation_lfo_amp, fb0_theta_saturation, 0)) /
              10 +
          c14 * .2f,
      (gui->fb0_bright + lfo(gui->fb0_bright_lfo_amp, fb0_theta_bright, 0)) /
              10 +
          c15 * .2f);

  shader_mixer.setUniform3f("fb0_hsb_x", hsb_x);

  hue_x.set((gui->fb0_huex_mod +
             lfo(gui->fb0_huexmod_lfo_amp, fb0_theta_huexmod, 0)) /
                    10 -
                c16,
            (gui->fb0_huex_offset +
             lfo(gui->fb0_huexoffset_lfo_amp, fb0_theta_huexoffset, 0)) /
                    10 +
                c17 * 2.0f,
            (gui->fb0_huex_lfo +
             lfo(gui->fb0_huexlfo_lfo_amp, fb0_theta_huexlfo, 0)) /
                    10 +
                c18 * 2.0f);

  shader_mixer.setUniform3f("fb0_hue_x", hue_x);

  fb_modswitch.set(gui->fb0_hue_invert, gui->fb0_saturation_invert,
                   gui->fb0_bright_invert);
  shader_mixer.setUniform3f("fb0_modswitch", fb_modswitch);

  shader_mixer.setUniform3f(
      "fb0_rescale",
      ofVec3f(gui->fb0_x_displace + lfo(gui->fb0_x_lfo_amp, fb0_theta_x, 0) +
                  c9 * 40.0f,
              gui->fb0_y_displace + lfo(gui->fb0_y_lfo_amp, fb0_theta_y, 0) +
                  c10 * 40.0f,
              gui->fb0_z_displace / 100.0f +
                  lfo(gui->fb0_z_lfo_amp, fb0_theta_z, 0) / 100.0f + c11));

  shader_mixer.setUniform1f(
      "fb0_rotate", (gui->fb0_rotate) +
                        lfo(gui->fb0_rotate_lfo_amp, fb0_theta_rotate, 0) +
                        c12);
  ...
  shader_mixer.setUniform3f(
      "tex_fb0_hsb_x",
      ofVec3f(gui->tex_fb0_hue, gui->tex_fb0_saturation, gui->tex_fb0_bright));

  shader_mixer.setUniform3f("tex_fb0_hue_x", ofVec3f(gui->tex_fb0_huex_mod,
                                                     gui->tex_fb0_huex_offset,
                                                     gui->tex_fb0_huex_lfo));
  // shader_mixer.setUniform3f("tex_fb0_hsb_x",ofVec3f(1.0,1.0,1.0));

  shader_mixer.setUniform3f("tex_fb0_rescale",
                            ofVec3f(gui->tex_fb0_x_displace,
                                    gui->tex_fb0_y_displace,
                                    gui->tex_fb0_z_displace / 100.0f));
  ...
  shader_mixer.setUniform1f(
      "fb1lumakeyvalue",
      gui->fb1_key_value + c19 +
          lfo(gui->fb1_lumakeyvalue_lfo_amp, fb1_theta_lumakeyvalue, 0));
  shader_mixer.setUniform1f("fb1lumakeythresh",
                            gui->fb1_key_threshold +
                                lfo(gui->fb1_lumakeythreshold_lfo_amp,
                                    fb1_theta_lumakeythreshold, 0));
  shader_mixer.setUniform1i("fb1mix", 2);
  shader_mixer.setUniform1i("fb1_toroid_switch", gui->fb1_toroid_switch);

  shader_mixer.setUniform1f(
      "fb1blend",
      gui->fb1_mix + lfo(gui->fb1_mix_lfo_amp, fb1_theta_mix, 0) + c20 * 2.0f);

  hsb_x.set(
      (gui->fb1_hue + lfo(gui->fb1_hue_lfo_amp, fb1_theta_hue, 0)) / 10 +
          c26 * .2f,
      (gui->fb1_saturation +
       lfo(gui->fb1_saturation_lfo_amp, fb1_theta_saturation, 0)) /
              10 +
          c27 * .2f,
      (gui->fb1_bright + lfo(gui->fb1_bright_lfo_amp, fb1_theta_bright, 0)) /
              10 +
          c28 * .2f);

  shader_mixer.setUniform3f("fb1_hsb_x", hsb_x);

  hue_x.set((gui->fb1_huex_mod +
             lfo(gui->fb1_huexmod_lfo_amp, fb1_theta_huexmod, 0)) /
                    10 -
                c29,
            (gui->fb1_huex_offset +
             lfo(gui->fb1_huexoffset_lfo_amp, fb1_theta_huexoffset, 0)) /
                    10 +
                c30 * 2.0f,
            (gui->fb1_huex_lfo +
             lfo(gui->fb1_huexlfo_lfo_amp, fb1_theta_huexlfo, 0)) /
                    10 +
                c31 * 2.0f);
  shader_mixer.setUniform3f("fb1_hue_x", hue_x);

  fb_modswitch.set(gui->fb1_hue_invert, gui->fb1_saturation_invert,
                   gui->fb1_bright_invert);
  shader_mixer.setUniform3f("fb1_modswitch", fb_modswitch);

  shader_mixer.setUniform3f(
      "fb1_rescale",
      ofVec3f(gui->fb1_x_displace + lfo(gui->fb1_x_lfo_amp, fb1_theta_x, 0) +
                  c9 * 40.0f,
              gui->fb1_y_displace + lfo(gui->fb1_y_lfo_amp, fb1_theta_y, 0) +
                  c10 * 40.0f,
              gui->fb1_z_displace / 100.0f +
                  lfo(gui->fb1_z_lfo_amp, fb1_theta_z, 0) / 100.0f + c24));
  shader_mixer.setUniform1f(
      "fb1_rotate", (gui->fb1_rotate) +
                        lfo(gui->fb1_rotate_lfo_amp, fb1_theta_rotate, 0) +
                        c12 * 6.18f);
  ...
  shader_mixer.setUniform3f(
      "tex_fb1_hsb_x",
      ofVec3f(gui->tex_fb1_hue, gui->tex_fb1_saturation, gui->tex_fb1_bright));

  shader_mixer.setUniform3f("tex_fb1_hue_x", ofVec3f(gui->tex_fb1_huex_mod,
                                                     gui->tex_fb1_huex_offset,
                                                     gui->tex_fb1_huex_lfo));
  // shader_mixer.setUniform3f("tex_fb1_hsb_x",ofVec3f(1.0,1.0,1.0));

  shader_mixer.setUniform3f("tex_fb1_rescale",
                            ofVec3f(gui->tex_fb1_x_displace,
                                    gui->tex_fb1_y_displace,
                                    gui->tex_fb1_z_displace / 100.0f));
  ...
  shader_mixer.setUniform1f(
      "fb2blend", gui->fb2_mix + lfo(gui->fb2_mix_lfo_amp, fb2_theta_mix, 0));

  hsb_x.set(
      (gui->fb2_hue + lfo(gui->fb2_hue_lfo_amp, fb2_theta_hue, 0)) / 10,
      (gui->fb2_saturation +
       lfo(gui->fb2_saturation_lfo_amp, fb2_theta_saturation, 0)) /
          10,
      (gui->fb2_bright + lfo(gui->fb2_bright_lfo_amp, fb2_theta_bright, 0)) /
          10);

  shader_mixer.setUniform3f("fb2_hsb_x", hsb_x);

  hue_x.set((gui->fb2_huex_mod +
             lfo(gui->fb2_huexmod_lfo_amp, fb2_theta_huexmod, 0)) /
                10,
            (gui->fb2_huex_offset +
             lfo(gui->fb2_huexoffset_lfo_amp, fb2_theta_huexoffset, 0)) /
                10,
            (gui->fb2_huex_lfo +
             lfo(gui->fb2_huexlfo_lfo_amp, fb2_theta_huexlfo, 0)) /
                10);
  shader_mixer.setUniform3f("fb2_hue_x", hue_x);

  fb_modswitch.set(gui->fb2_hue_invert, gui->fb2_saturation_invert,
                   gui->fb2_bright_invert);
  shader_mixer.setUniform3f("fb2_modswitch", fb_modswitch);

  shader_mixer.setUniform3f(
      "fb2_rescale",
      ofVec3f(gui->fb2_x_displace + lfo(gui->fb2_x_lfo_amp, fb2_theta_x, 0) +
                  c9 * 40.0f,
              gui->fb2_y_displace + lfo(gui->fb2_y_lfo_amp, fb2_theta_y, 0) +
                  c10 * 40.0f,
              gui->fb2_z_displace / 100.0f +
                  lfo(gui->fb2_z_lfo_amp, fb2_theta_z, 0) / 100.0f));

  shader_mixer.setUniform1f(
      "fb2_rotate",
      (gui->fb2_rotate) + lfo(gui->fb2_rotate_lfo_amp, fb2_theta_rotate, 0));
  ...
  shader_mixer.setUniform3f(
      "tex_fb2_hsb_x",
      ofVec3f(gui->tex_fb2_hue, gui->tex_fb2_saturation, gui->tex_fb2_bright));

  shader_mixer.setUniform3f("tex_fb2_hue_x", ofVec3f(gui->tex_fb2_huex_mod,
                                                     gui->tex_fb2_huex_offset,
                                                     gui->tex_fb2_huex_lfo));
  // shader_mixer.setUniform3f("tex_fb2_hsb_x",ofVec3f(1.0,1.0,1.0));

  shader_mixer.setUniform3f("tex_fb2_rescale",
                            ofVec3f(gui->tex_fb2_x_displace,
                                    gui->tex_fb2_y_displace,
                                    gui->tex_fb2_z_displace / 100.0f));
  ...
  shader_mixer.setUniform1f(
      "fb3blend", gui->fb3_mix + lfo(gui->fb3_mix_lfo_amp, fb3_theta_mix, 0));

  hsb_x.set(
      (gui->fb3_hue + lfo(gui->fb3_hue_lfo_amp, fb3_theta_hue, 0)) / 10,
      (gui->fb3_saturation +
       lfo(gui->fb3_saturation_lfo_amp, fb3_theta_saturation, 0)) /
          10,
      (gui->fb3_bright + lfo(gui->fb3_bright_lfo_amp, fb3_theta_bright, 0)) /
          10);

  shader_mixer.setUniform3f("fb3_hsb_x", hsb_x);

  hue_x.set((gui->fb3_huex_mod +
             lfo(gui->fb3_huexmod_lfo_amp, fb3_theta_huexmod, 0)) /
                10,
            (gui->fb3_huex_offset +
             lfo(gui->fb3_huexoffset_lfo_amp, fb3_theta_huexoffset, 0)) /
                10,
            (gui->fb3_huex_lfo +
             lfo(gui->fb3_huexlfo_lfo_amp, fb3_theta_huexlfo, 0)) /
                10);
  shader_mixer.setUniform3f("fb3_hue_x", hue_x);

  fb_modswitch.set(gui->fb3_hue_invert, gui->fb3_saturation_invert,
                   gui->fb3_bright_invert);
  shader_mixer.setUniform3f("fb3_modswitch", fb_modswitch);

  shader_mixer.setUniform3f(
      "fb3_rescale",
      ofVec3f(gui->fb3_x_displace + lfo(gui->fb3_x_lfo_amp, fb3_theta_x, 0),
              gui->fb3_y_displace + lfo(gui->fb3_y_lfo_amp, fb3_theta_y, 0),
              gui->fb3_z_displace / 100 +
                  lfo(gui->fb3_z_lfo_amp, fb3_theta_z, 0) / 100.0f));

  shader_mixer.setUniform1f(
      "fb3_rotate",
      (gui->fb3_rotate) + lfo(gui->fb3_rotate_lfo_amp, fb3_theta_rotate, 0));
  ...
  shader_mixer.setUniform3f(
      "tex_fb3_hsb_x",
      ofVec3f(gui->tex_fb3_hue, gui->tex_fb3_saturation, gui->tex_fb3_bright));

  shader_mixer.setUniform3f("tex_fb3_hue_x", ofVec3f(gui->tex_fb3_huex_mod,
                                                     gui->tex_fb3_huex_offset,
                                                     gui->tex_fb3_huex_lfo));
  // shader_mixer.setUniform3f("tex_fb3_hsb_x",ofVec3f(1.0,1.0,1.0));

  shader_mixer.setUniform3f("tex_fb3_rescale",
                            ofVec3f(gui->tex_fb3_x_displace,
                                    gui->tex_fb3_y_displace,
                                    gui->tex_fb3_z_displace / 100.0f));
  ...
  shader_mixer.setUniform1f("channel1hue_x",
                            gui->ch1_hue + c1 * 5.0 +
                                lfo(gui->ch1_hue_lfo_amp, ch1_theta_hue, 0));
  shader_mixer.setUniform1f(
      "channel1saturation_x",
      gui->ch1_saturation + c2 * 5.0 +
          lfo(gui->ch1_saturation_lfo_amp, ch1_theta_saturation, 0));
  shader_mixer.setUniform1f(
      "channel1bright_x",
      gui->ch1_bright + c3 * 5.0 +
          abs(lfo(gui->ch1_bright_lfo_amp, ch1_theta_bright, 0)));
  ...
  shader_mixer.setUniform1i("ch1hue_inverttoggle",
                            gui->ch1_hue_alt_invert_toggle);
  shader_mixer.setUniform1i("ch1sat_inverttoggle",
                            gui->ch1_saturation_alt_invert_toggle);
  shader_mixer.setUniform1i("ch1bright_inverttoggle",
                            gui->ch1_bright_alt_invert_toggle);
  ...
  shader_mixer.setUniform1f(
      "channel2bright_x",
      gui->ch2_bright + lfo(gui->ch2_bright_lfo_amp, ch2_theta_bright, 0));
  shader_mixer.setUniform1f("channel2hue_x",
                            gui->ch2_hue +
                                lfo(gui->ch2_hue_lfo_amp, ch2_theta_hue, 0));
  shader_mixer.setUniform1f(
      "channel2saturation_x",
      gui->ch2_saturation +
          lfo(gui->ch2_saturation_lfo_amp, ch2_theta_saturation, 0));
  ...
  shader_mixer.setUniform1i("ch2hue_inverttoggle",
                            gui->ch2_hue_alt_invert_toggle);
  shader_mixer.setUniform1i("ch2sat_inverttoggle",
                            gui->ch2_saturation_alt_invert_toggle);
  shader_mixer.setUniform1i("ch2bright_inverttoggle",
                            gui->ch2_bright_alt_invert_toggle);
  ...
  shader_mixer.setUniform2f(
      "cam1dimensions",
      ofVec2f(cam1 ? cam1->getWidth() : 0, cam1 ? cam1->getHeight() : 0));
  shader_mixer.setUniform2f(
      "cam2dimensions",
      ofVec2f(cam2 ? cam2->getWidth() : 0, cam2 ? cam2->getHeight() : 0));
  ...
  shader_mixer.setUniform1i(
      "cam1_pixel_scale_x",
      gui->cam1_pixel_scale_x +
          lfo(gui->cam1_pixel_scale_x_lfo_amp, cam1_theta_pixel_scale_x, 0));
  shader_mixer.setUniform1i(
      "cam1_pixel_scale_y",
      gui->cam1_pixel_scale_y +
          lfo(gui->cam1_pixel_scale_y_lfo_amp, cam1_theta_pixel_scale_y, 0));
  shader_mixer.setUniform1f(
      "cam1_pixel_mix", gui->cam1_pixel_mix + lfo(gui->cam1_pixel_mix_lfo_amp,
                                                  cam1_theta_pixel_mix, 0));
  shader_mixer.setUniform1f("cam1_pixel_brightscale",
                            gui->cam1_pixel_brightscale +
                                lfo(gui->cam1_pixel_brightscale_lfo_amp,
                                    cam1_theta_pixel_brightscale, 0));
  ...
  shader_mixer.setUniform1i(
      "cam2_pixel_scale_x",
      gui->cam2_pixel_scale_x +
          lfo(gui->cam2_pixel_scale_x_lfo_amp, cam2_theta_pixel_scale_x, 0));
  shader_mixer.setUniform1i(
      "cam2_pixel_scale_y",
      gui->cam2_pixel_scale_y +
          lfo(gui->cam2_pixel_scale_y_lfo_amp, cam2_theta_pixel_scale_y, 0));
  shader_mixer.setUniform1f(
      "cam2_pixel_mix", gui->cam2_pixel_mix + lfo(gui->cam2_pixel_mix_lfo_amp,
                                                  cam2_theta_pixel_mix, 0));
  shader_mixer.setUniform1f("cam2_pixel_brightscale",
                            gui->cam2_pixel_brightscale +
                                lfo(gui->cam2_pixel_brightscale_lfo_amp,
                                    cam2_theta_pixel_brightscale, 0));

  // fb0
  shader_mixer.setUniform1i("fb0_pixel_switch", gui->fb0_pixel_switch);
  shader_mixer.setUniform1i(
      "fb0_pixel_scale_x",
      gui->fb0_pixel_scale_x +
          lfo(gui->fb0_pixel_scale_x_lfo_amp, fb0_theta_pixel_scale_x, 0));
  shader_mixer.setUniform1i(
      "fb0_pixel_scale_y",
      gui->fb0_pixel_scale_y +
          lfo(gui->fb0_pixel_scale_y_lfo_amp, fb0_theta_pixel_scale_y, 0));
  shader_mixer.setUniform1f("fb0_pixel_mix",
                            gui->fb0_pixel_mix + lfo(gui->fb0_pixel_mix_lfo_amp,
                                                     fb0_theta_pixel_mix, 0));
  shader_mixer.setUniform1f("fb0_pixel_brightscale",
                            gui->fb0_pixel_brightscale +
                                lfo(gui->fb0_pixel_brightscale_lfo_amp,
                                    fb0_theta_pixel_brightscale, 0));
  ...
  shader_mixer.setUniform1i("texmod_fb0_pixel_scale_x",
                            gui->texmod_fb0_pixel_scale_x);
  shader_mixer.setUniform1i("texmod_fb0_pixel_scale_y",
                            gui->texmod_fb0_pixel_scale_y);
  shader_mixer.setUniform1f("texmod_fb0_pixel_mix", gui->texmod_fb0_pixel_mix);
  shader_mixer.setUniform1f("texmod_fb0_pixel_brightscale",
                            gui->texmod_fb0_pixel_brightscale);

  // fb1
  shader_mixer.setUniform1i("fb1_pixel_switch", gui->fb1_pixel_switch);
  shader_mixer.setUniform1i(
      "fb1_pixel_scale_x",
      gui->fb1_pixel_scale_x +
          lfo(gui->fb1_pixel_scale_x_lfo_amp, fb1_theta_pixel_scale_x, 0));
  shader_mixer.setUniform1i(
      "fb1_pixel_scale_y",
      gui->fb1_pixel_scale_y +
          lfo(gui->fb1_pixel_scale_y_lfo_amp, fb1_theta_pixel_scale_y, 0));
  shader_mixer.setUniform1f("fb1_pixel_mix",
                            gui->fb1_pixel_mix + lfo(gui->fb1_pixel_mix_lfo_amp,
                                                     fb1_theta_pixel_mix, 0));
  shader_mixer.setUniform1f("fb1_pixel_brightscale",
                            gui->fb1_pixel_brightscale +
                                lfo(gui->fb1_pixel_brightscale_lfo_amp,
                                    fb1_theta_pixel_brightscale, 0));
  ...
  shader_mixer.setUniform1i("texmod_fb1_pixel_scale_x",
                            gui->texmod_fb1_pixel_scale_x);
  shader_mixer.setUniform1i("texmod_fb1_pixel_scale_y",
                            gui->texmod_fb1_pixel_scale_y);
  shader_mixer.setUniform1f("texmod_fb1_pixel_mix", gui->texmod_fb1_pixel_mix);
  shader_mixer.setUniform1f("texmod_fb1_pixel_brightscale",
                            gui->texmod_fb1_pixel_brightscale);

  // fb2
  shader_mixer.setUniform1i("fb2_pixel_switch", gui->fb2_pixel_switch);
  shader_mixer.setUniform1i(
      "fb2_pixel_scale_x",
      gui->fb2_pixel_scale_x +
          lfo(gui->fb2_pixel_scale_x_lfo_amp, fb2_theta_pixel_scale_x, 0));
  shader_mixer.setUniform1i(
      "fb2_pixel_scale_y",
      gui->fb2_pixel_scale_y +
          lfo(gui->fb2_pixel_scale_y_lfo_amp, fb2_theta_pixel_scale_y, 0));
  shader_mixer.setUniform1f("fb2_pixel_mix",
                            gui->fb2_pixel_mix + lfo(gui->fb2_pixel_mix_lfo_amp,
                                                     fb2_theta_pixel_mix, 0));
  shader_mixer.setUniform1f("fb2_pixel_brightscale",
                            gui->fb2_pixel_brightscale +
                                lfo(gui->fb2_pixel_brightscale_lfo_amp,
                                    fb2_theta_pixel_brightscale, 0));
  ...
  shader_mixer.setUniform1i("texmod_fb2_pixel_scale_x",
                            gui->texmod_fb2_pixel_scale_x);
  shader_mixer.setUniform1i("texmod_fb2_pixel_scale_y",
                            gui->texmod_fb2_pixel_scale_y);
  shader_mixer.setUniform1f("texmod_fb2_pixel_mix", gui->texmod_fb2_pixel_mix);
  shader_mixer.setUniform1f("texmod_fb2_pixel_brightscale",
                            gui->texmod_fb2_pixel_brightscale);

  // fb3
  shader_mixer.setUniform1i("fb3_pixel_switch", gui->fb3_pixel_switch);
  shader_mixer.setUniform1i(
      "fb3_pixel_scale_x",
      gui->fb3_pixel_scale_x +
          lfo(gui->fb3_pixel_scale_x_lfo_amp, fb3_theta_pixel_scale_x, 0));
  shader_mixer.setUniform1i(
      "fb3_pixel_scale_y",
      gui->fb3_pixel_scale_y +
          lfo(gui->fb3_pixel_scale_y_lfo_amp, fb3_theta_pixel_scale_y, 0));
  shader_mixer.setUniform1f("fb3_pixel_mix",
                            gui->fb3_pixel_mix + lfo(gui->fb3_pixel_mix_lfo_amp,
                                                     fb3_theta_pixel_mix, 0));
  shader_mixer.setUniform1f("fb3_pixel_brightscale",
                            gui->fb3_pixel_brightscale +
                                lfo(gui->fb3_pixel_brightscale_lfo_amp,
                                    fb3_theta_pixel_brightscale, 0));
  ...
  shader_mixer.setUniform1i("texmod_fb3_pixel_scale_x",
                            gui->texmod_fb3_pixel_scale_x);
  shader_mixer.setUniform1i("texmod_fb3_pixel_scale_y",
                            gui->texmod_fb3_pixel_scale_y);
  shader_mixer.setUniform1f("texmod_fb3_pixel_mix", gui->texmod_fb3_pixel_mix);
  shader_mixer.setUniform1f("texmod_fb3_pixel_brightscale",
                            gui->texmod_fb3_pixel_brightscale);
  shader_mixer.end();
  ...
  if (gui->tetrahedron_switch == 1) {
    ...
    ofSetColor(127 + 127 * (sin(ofGetElapsedTimef())),
               127 + 127 * (cos(ofGetElapsedTimef() / 7)),
               127 - 127 * (sin(ofGetElapsedTimef() / 19)), 255);
    ofNoFill();
    ofPushMatrix();
  ...
  shader_blur.setUniform1f("blur_amount",
                           gui->blur_amount + c4 * 10 +
                               lfo(gui->blur_amount_lfo_amp, blur_theta_amount,
                                   0)); //+5*(midi_controls[25]+1)/2);
  shader_blur.setUniform1f(
      "blur_radius",
      gui->blur_radius + lfo(gui->blur_radius_lfo_amp, blur_theta_radius, 0));
  ...
  shader_sharpen.setUniform1f("texmod_sharpen_amount",
                              gui->texmod_sharpen_amount);
  shader_sharpen.setUniform1f("texmod_sharpen_radius",
                              gui->texmod_sharpen_radius);
  shader_sharpen.setUniform1f("texmod_sharpen_boost",
                              gui->texmod_sharpen_boost);

  // lets change sharpAMNT to be larger?
  shader_sharpen.setUniform1f("sharpen_amount",
                              gui->sharpen_amount + c5 * .3 +
                                  lfo(gui->sharpen_amount_lfo_amp,
                                      sharpen_theta_amount,
                                      0)); //+.3*(midi_controls[24]+1.0)/2.0);
  shader_sharpen.setUniform1f(
      "sharpen_radius", gui->sharpen_radius + lfo(gui->sharpen_radius_lfo_amp,
                                                  sharpen_theta_radius, 0));
  shader_sharpen.setUniform1f(
      "sharpen_boost",
      gui->sharpen_boost + c5 +
          lfo(gui->sharpen_boost_lfo_amp, sharpen_theta_boost, 0));
  ...
  //  ofDrawBitmapString(" delayhead: "+ofToString(delayhead),10,
  //  ofGetHeight()-5 );
...
float ofApp::lfo(float amp, float rate, int shape) { return amp * sin(rate); }
...
void ofApp::hypercube_draw() {
  ...
  for (int i = 0; i < limit; i++) {
    ...
    hypercube_x[0] = xr * (cos(hypercube_theta) - sin(hypercube_theta)) *
                     (1 - .5 * (cos(hypercube_phi)));
    hypercube_x[1] = xr * (cos(hypercube_theta) + sin(hypercube_theta)) *
                     (1 - .5 * (cos(PI / 4 + hypercube_phi)));
    hypercube_x[2] = xr * (-cos(hypercube_theta) + sin(hypercube_theta)) *
                     (1 - .5 * (cos(PI / 2 + hypercube_phi)));
    hypercube_x[3] = xr * (-cos(hypercube_theta) - sin(hypercube_theta)) *
                     (1 - .5 * (cos(3 * PI / 4 + hypercube_phi)));
    hypercube_x[4] = xr * (cos(hypercube_theta) - sin(hypercube_theta)) *
                     (1 - .5 * (cos(PI + hypercube_phi)));
    hypercube_x[5] = xr * (cos(hypercube_theta) + sin(hypercube_theta)) *
                     (1 - .5 * (cos(5 * PI / 4 + hypercube_phi)));
    hypercube_x[6] = xr * (-cos(hypercube_theta) + sin(hypercube_theta)) *
                     (1 - .5 * (cos(3 * PI / 2 + hypercube_phi)));
    hypercube_x[7] = xr * (-cos(hypercube_theta) - sin(hypercube_theta)) *
                     (1 - .5 * (cos(7 * PI / 4 + hypercube_phi)));

    float yr = hypercube_r * (1);
    hypercube_y[0] = yr * (sin(hypercube_theta) + cos(hypercube_theta)) *
                     (1 - .5 * (cos(hypercube_phi)));
    hypercube_y[1] = yr * (sin(hypercube_theta) - cos(hypercube_theta)) *
                     (1 - .5 * (cos(PI / 4 + hypercube_phi)));
    hypercube_y[2] = yr * (-sin(hypercube_theta) - cos(hypercube_theta)) *
                     (1 - .5 * (cos(PI / 2 + hypercube_phi)));
    hypercube_y[3] = yr * (-sin(hypercube_theta) + cos(hypercube_theta)) *
                     (1 - .5 * (cos(3 * PI / 4 + hypercube_phi)));
    hypercube_y[4] = yr * (sin(hypercube_theta) + cos(hypercube_theta)) *
                     (1 - .5 * (cos(PI + hypercube_phi)));
    hypercube_y[5] = yr * (sin(hypercube_theta) - cos(hypercube_theta)) *
                     (1 - .5 * (cos(5 * PI / 4 + hypercube_phi)));
    hypercube_y[6] = yr * (-sin(hypercube_theta) - cos(hypercube_theta)) *
                     (1 - .5 * (cos(3 * PI / 2 + hypercube_phi)));
    hypercube_y[7] = yr * (-sin(hypercube_theta) + cos(hypercube_theta)) *
                     (1 - .5 * (cos(7 * PI / 4 + hypercube_phi)));
    ...
    color_theta += .01;
    ofSetColor(127 + 127 * sin(color_theta),
               0 + 192 * abs(cos(color_theta * .2)),
               127 + 127 * cos(color_theta / 3.0f));
    ofNoFill();
    ofPushMatrix();
    ofTranslate(ofGetWidth() / 2, ofGetHeight() / 2);
    ofRotateYRad(-PI / 2);
    ...
    ofDrawLine(hypercube_x[0], hypercube_y[0], hypercube_z[0], hypercube_x[1],
               hypercube_y[1], hypercube_z[1]);
    ofDrawLine(hypercube_x[1], hypercube_y[1], hypercube_z[1], hypercube_x[2],
               hypercube_y[2], hypercube_z[2]);
    ofDrawLine(hypercube_x[2], hypercube_y[2], hypercube_z[2], hypercube_x[3],
               hypercube_y[3], hypercube_z[3]);
    ofDrawLine(hypercube_x[3], hypercube_y[3], hypercube_z[3], hypercube_x[4],
               hypercube_y[4], hypercube_z[4]);
    ofDrawLine(hypercube_x[4], hypercube_y[4], hypercube_z[4], hypercube_x[5],
               hypercube_y[5], hypercube_z[5]);
    ofDrawLine(hypercube_x[5], hypercube_y[5], hypercube_z[5], hypercube_x[6],
               hypercube_y[6], hypercube_z[6]);
    ofDrawLine(hypercube_x[6], hypercube_y[6], hypercube_z[6], hypercube_x[7],
               hypercube_y[7], hypercube_z[7]);
    ofDrawLine(hypercube_x[7], hypercube_y[7], hypercube_z[7], hypercube_x[0],
               hypercube_y[0], hypercube_z[0]);
    ...
    ofDrawLine(hypercube_x[0], hypercube_y[0], hypercube_z[0], hypercube_x[0],
               hypercube_y[0], -hypercube_z[0]);
    ofDrawLine(hypercube_x[1], hypercube_y[1], hypercube_z[1], hypercube_x[1],
               hypercube_y[1], -hypercube_z[1]);
    ofDrawLine(hypercube_x[2], hypercube_y[2], hypercube_z[2], hypercube_x[2],
               hypercube_y[2], -hypercube_z[2]);
    ofDrawLine(hypercube_x[3], hypercube_y[3], hypercube_z[3], hypercube_x[3],
               hypercube_y[3], -hypercube_z[3]);
    ofDrawLine(hypercube_x[4], hypercube_y[4], hypercube_z[4], hypercube_x[4],
               hypercube_y[4], -hypercube_z[4]);
    ofDrawLine(hypercube_x[5], hypercube_y[5], hypercube_z[5], hypercube_x[5],
               hypercube_y[5], -hypercube_z[5]);
    ofDrawLine(hypercube_x[6], hypercube_y[6], hypercube_z[6], hypercube_x[6],
               hypercube_y[6], -hypercube_z[6]);
    ofDrawLine(hypercube_x[7], hypercube_y[7], hypercube_z[7], hypercube_x[7],
               hypercube_y[7], -hypercube_z[7]);

    ofDrawLine(hypercube_x[0], hypercube_y[0], -hypercube_z[0], hypercube_x[4],
               hypercube_y[4], -hypercube_z[4]);
    ofDrawLine(hypercube_x[1], hypercube_y[1], -hypercube_z[1], hypercube_x[5],
               hypercube_y[5], -hypercube_z[5]);
    ofDrawLine(hypercube_x[2], hypercube_y[2], -hypercube_z[2], hypercube_x[6],
               hypercube_y[6], -hypercube_z[6]);
    ofDrawLine(hypercube_x[3], hypercube_y[3], -hypercube_z[3], hypercube_x[7],
               hypercube_y[7], -hypercube_z[7]);
...
void ofApp::keyPressed(int key) {
  ...
  if (key == '9') {
    cam1Manager.incrementDeviceID();
  }
  if (key == '0') {
    cam2Manager.incrementDeviceID();
  }
  // if(key=='q'){sw1==0;}
  ...
  for (unsigned int i = 0; i < midiMessages.size(); ++i) {
    ...
    if (message.status < MIDI_SYSEX) {
      ...
      if (message.status == MIDI_CONTROL_CHANGE) {
        ...
        // MIDIMAPZONE
        // these are all set to output bipolor controls at this moment (ranging
        // from -1.0 to 1.0) if u uncomment the second line on each of these if
        // statements that will switch thems to unipolor controls (ranging from
        // 0.0to 1.0) if  you prefer then find the variable that youd like to
        // control down in CAVARIABLEZONES or MIXERVARIBLEZONES and substitute
        // c1,c2, ..cn whichever control knob u wish the map
      }
